...
object AnrCatalog {
    val scenarios: List<AnrScenario> = listOf(
        ...
        AnrScenario(
            id = "binder-peer-stall",
            title = "Binder 对端卡顿 / 同步 IPC 等待",
            category = AnrCategory.UserPerceived,
            riskLevel = AnrRiskLevel.Medium,
            triggerKind = AnrTriggerKind.BinderPeer,
            timeoutDescription = "Binder 本身没有固定 ANR 阈值；Demo 让主线程同步等待远端进程 8 秒，再按 Input dispatch 5 秒观察。",
            recommendedBlockMs = AnrDefaults.BINDER_PEER_BLOCK_MS,
            expectedReason = "Input dispatching timed out; main thread waiting in BinderProxy.transactNative",
            explanation = "根因场景：Activity 主线程向独立进程服务发起同步 Binder transact，对端 Binder 线程故意睡眠，调用方主线程一直等远端回复。",
            defaultRequest = AnrTriggerRequest("binder-peer-stall", blockMs = AnrDefaults.BINDER_PEER_BLOCK_MS),
            adbCommand = deepLink("binder-peer-stall", "blockMs=${AnrDefaults.BINDER_PEER_BLOCK_MS}"),
            parameterSpecs = listOf(duration("远端 Binder 阻塞", AnrDefaults.BINDER_PEER_BLOCK_MS, 5_000, 30_000)),
        ),
        AnrScenario(
            id = "short-service",
